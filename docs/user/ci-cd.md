...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Install odcs
        run: cargo install odcs --version 0.4.0 --locked
...
- Pin the **published** tool version in CI for reproducibility. Latest published is **0.4.0** until `v0.5.0` ships — see [Release status](../project/release-status.md).
- After `v0.5.0` is published, update pins to `cargo install odcs --version 0.5.0 --locked` and `pip install pyodcs==0.5.0`.
